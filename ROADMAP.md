...
- **Interest-theme polish (post Phase 2)** — core + auto-personalized default shipped. Remaining nice-to-haves: cross-device interest sync via sign-in (localStorage-only today); lower the ~24% no-theme classifier rate; add a real "Sports & active" data source (thin today); window the chip counts to the visible set. _Effort: M._
...
- **Repo cleanup: root screenshots, tracked drift, dead code** — ~35 PNG screenshots at repo root + routine event-data drift; plus the unreachable `view === "home"` branch in `App.tsx` (the "This weekend" highlight cards — `readAppRoute()` never returns `home` and nothing calls `setView("home")`). Either wire home up or delete it. _Effort: XS–S._
