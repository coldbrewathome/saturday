...
const uploadManifest = {
  ...
  shared: {
    ...
    playlist: "This Weekend With Kids",
  },
  shorts: [],
};
...
for (const m of M.metros) {
  ...
  out.push("```");
  const desc = [
    `${Win} in ${the}${m.name}, from the venues' own calendars. All ${m.weekTotal} of them: ${url}`,
    "",
  ];
  ...
  for (const [i, p] of m.picks.entries()) {
    ...
    desc.push(`${mark} · ${p.title} — ${p.venue}, ${p.city} — ${day} ${p.time} — ${p.cost}${p.ages ? ` — ${p.ages}` : ""}`);
  }
  desc.push("");
  ...
  desc.push("");
  desc.push(`${HASHTAG[m.id]} #thingstodowithkids #familyactivities`);
  out.push(...desc);
  out.push("```");
  out.push("");
  out.push("**Tags**");
  out.push("```");
  const tags = [...SEARCH_TERMS[m.id], "family events near me", "famhop"];
  out.push(tags.join(", "));
  out.push("```");
  out.push("");
  ...
  uploadManifest.shorts.push({
    id: m.id,
    name: m.name,
    file: `videos/delivery-${M.weekend.saturday}/famhop-${m.id}-${FILE_TAG}.mp4`,
    title,
...
writeFileSync(join(HERE, SHORTS_DIR, "upload-manifest.json"), JSON.stringify(uploadManifest, null, 2));
console.log(`wrote videos/${SHORTS_DIR}/YOUTUBE-METADATA.md`);
console.log(`wrote videos/${SHORTS_DIR}/upload-manifest.json`);
