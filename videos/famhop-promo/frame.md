...
description: >
  ...
  borders, hard offset shadows (no blur), Bricolage Grotesque 800 uppercase at 0.92 line-height,
  JetBrains Mono taxonomy, Bricolage Grotesque body, the four-accent palette. Composition,
...
colors:
  cream: "#FAF5EB"       # --bg
  cream-2: "#F2EAD9"     # --surface-strong
  ink: "#1B1916"         # --ink
  ink-2: "#4A453F"       # --ink-soft
  green: "#2D5043"       # --forest
  green-dark: "#1F3A2F"  # --forest-strong
  pink: "#B25368"        # --berry
  pink-dark: "#8C3F51"   # berry, darkened for rules on berry
  orange: "#DD6A1A"      # --accent (primary brand accent + hard-shadow color)
  yellow: "#E8B547"      # --sun

typography:
  # — reading ramp (px @ 1920, with cqw) —
  body-lg:    { fontFamily: "Plus Jakarta Sans", px: 28, cqw: 1.46, weight: 400, lineHeight: 1.4 }
  body-md:    { fontFamily: "Plus Jakarta Sans", px: 24, cqw: 1.25, weight: 400, lineHeight: 1.3 }
  ...
  table-head: { fontFamily: "Bricolage Grotesque", px: 28, cqw: 1.46, weight: 800, upper: true }
  # — display / hero ramp (frame-native, cqw-first) —
  step-title: { fontFamily: "Bricolage Grotesque", px: 34,  cqw: 1.77, weight: 800, lineHeight: 1.0, upper: true }
  badge-label:{ fontFamily: "Bricolage Grotesque", px: 28,  cqw: 1.46, weight: 800, upper: true }
  marker:     { fontFamily: "Bricolage Grotesque", cqw: 2.4, weight: 800, lineHeight: 1.0, upper: true }
  stamp-num:  { fontFamily: "Bricolage Grotesque", cqw: 2.2, weight: 800, lineHeight: 0.9 }
  stat-num:   { fontFamily: "Bricolage Grotesque", cqw: 6.4, weight: 800, lineHeight: 0.88 }
  step-num:   { fontFamily: "Bricolage Grotesque", cqw: 7.0, weight: 800, lineHeight: 0.85 }
  display-head: { fontFamily: "Bricolage Grotesque", cqw: 4.2, weight: 800, lineHeight: 0.92, tracking: "-0.01em", upper: true }
  display-lg: { fontFamily: "Bricolage Grotesque", cqw: 8.0,  weight: 800, lineHeight: 0.9,  tracking: "-0.01em", upper: true }
  display-xl: { fontFamily: "Bricolage Grotesque", cqw: 11.0, weight: 800, lineHeight: 0.9,  tracking: "-0.01em", upper: true }
  display-hero:{ fontFamily: "Bricolage Grotesque", cqw: 15.5,weight: 800, lineHeight: 0.84, tracking: "-0.02em", upper: true }
...
components:
  ...
  comparison-table:
    ...
    shadow: "none"
    description: "Ink head row with cream Bricolage Grotesque 800 labels; pink/green column-fill variants."
  decorative-circle:
    backgroundColor: "{colors.yellow}"
    border: "0.4cqw solid {colors.ink}"
    rounded: "50%"
...
- **Fonts** — **Bricolage Grotesque 800** (display, uppercase) / **Plus Jakarta Sans 400** (body) / **JetBrains Mono 400** (labels, kickers, counters). These are famhop.com's own faces; the preset's Archivo Black is gone. Load from Google Fonts.
- **Weights** — the brand fonts ship `{400, 500, 600, 700, 800}`; clamp to these. Display is always 800, body always 400.
- **Colors** — use the frontmatter hex. The four accents are FamHop's real brand tokens: `green` = forest `#2D5043`, `pink` = berry `#B25368`, `orange` = the primary accent `#DD6A1A`, `yellow` = sun `#E8B547`. Preset color NAMES in the prose below mean these remapped values — "pink" is berry, "green" is forest.
- **Logo** — the wordmark is `assets/logo-famhop.svg` (fills use `currentColor` + `var(--accent)`, so set `--accent: #DD6A1A` on any ancestor). The rounded tile mark is `assets/logo-famhop-tile.svg`. Never redraw the logotype in live text.
...
The display voice is **Bricolage Grotesque 800 in strict uppercase at 0.92 line-height** — letters overlap
their own cap height; that tightness is the brand. **JetBrains Mono** carries every label, kicker,
counter, and axis as a "technical artifact" register. **Plus Jakarta Sans** carries the few body lines.
...
- **Bricolage Grotesque 800 uppercase**, 0.92 line-height, always; sentence-case Bricolage Grotesque 800 does not exist.
...
- **Uppercase + 0.92 line-height on all Bricolage Grotesque 800**, always. Mono carries 0.06–0.14em tracking. Never letter-space Bricolage Grotesque 800 beyond the encoded −0.01em; never set body centered.
...
**Fixed** Bricolage Grotesque 800 uppercase 0.84 lh, one accent on the wordmark, square corners. **Free** which
...
cream-2 fill, 0.3cqw internal rules, ink head row with cream Bricolage Grotesque 800 labels. **Free** rows, which
...
- Size headlines **fit-to-measure**; render Bricolage Grotesque 800 uppercase at 0.92 lh.

### Don't

- Don't round corners (except the pill chip); don't gradient, blur, or glow.
- Don't set Bricolage Grotesque 800 in sentence case or letter-space it beyond −0.01em.
...
- **Type** — Bricolage Grotesque 800 uppercase 0.92 lh, fit-to-measure, ≥1.4cqw on load-bearing lines.
...
- **Bricolage Grotesque 800 requires Google Fonts**; fallback is `sans-serif`. CJK pairing (Noto Serif SC 900 / NSC 400) carries over from the source's CJK section.
